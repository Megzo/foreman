...
## Phase 6: In-task chat, requestUserInput modal, cancel — ✅ COMPLETE (2026-06-12)

> Done: adapter `startTurn` now returns `{turnId}` (schema: V2TurnStartResponse) and `InterruptTurnOptions.turnId` is required (schema fix); `TaskRunner` gained `sendChat` (steer with `expectedTurnId` while a turn is in progress, `turn/start` on the same thread when idle — FR-4.3), `cancel()` (`turn/interrupt` → `turn.status "interrupted"` → new `"cancelled"` terminal state, FR-4.5/4.6), and an `onUserInput` option (default stays first-option so headless runs never hang); main↔renderer IPC gained `sendChat`/`cancelTask`/`onUserInputRequest`/`answerUserInput` with requestId correlation in main; renderer gained `ChatPane` (per-run transcript in renderer state), `UserInputModal` (options→radio, no options→free text, protocol-shaped `{answers:{id:{answers:[...]}}}` payload), `ConfirmDialog`, RunView "Megszakítva" state, all wired in `TaskScreen`. Mock peer gained `steerable` (turn idles open; steer/interrupt validate the turn-id precondition like the real server) and `user-input` scenarios; `FOREMAN_MOCK_SCENARIO` env knob lets dev/E2E pick scenarios. echo-demo SKILL.md asks a request_user_input confirmation question ("Igen" first, so the fallback still writes the file) — the manual modal test asset. Fixed a real race caught by a flaky test: `turn/completed` arriving in the same stdio chunk as the `turn/start` response could resurrect a closed turn's id; guarded in launch/sendChat, full suite then ran 5× green. 112 vitest tests + 4 E2E (steer, cancel, modal flows added) + `pnpm typecheck`/`build`/`smoke` (real codex SPIKE_OK) all green. Carry-forward: Open Q4 (real-codex steer reliability mid-skill) is still unresolved empirically — the manual real-codex run (steer mid-run, answer the skill's modal question, cancel a fresh run) needs Peter at a signed-in session, alongside the Phase 3/4/5 walk-throughs; if steering proves unreliable, implement the documented queue-fallback.
...
## Phase 7: Sessions, history, resume, crash recovery — ✅ COMPLETE (2026-06-12)

> Done: `SessionStore` (run-centric public API `createRun/recordThread/appendEvent/finishRun/listRuns/getRun/findResumable/readTranscript`, 5 tests) — **fs-backed, not better-sqlite3**: a JSON index + one JSONL transcript per run under `userData/sessions/`, `schemaVersion`-stamped from day one. The plan/PRD recommended SQLite, but the plan also fixed the store API as the public interface and "the SQL is not"; a native module would need an Electron-ABI rebuild that breaks the direct-launch E2E and is the first native dep in an otherwise pure-fs repo, so I kept it fs (swappable to SQLite later with zero test changes — flag for Peter if he wants the native backend). `TaskRunner` gained `store?`, persists the full lifecycle + transcript (notify-then-persist so the disk never gates delta latency, FR-2.4), `resume(runId)` (thread/resume the stored thread → re-invoke the skill turn into the same run record) and `activeRunId()`. Adapter restart+resume now exercised (start() again after a death → `resumeThread`, FR-2.5 recovery half); mock peer handles `thread/resume` (rejects any id but the one mock thread, so a resumed turn proves the stored id went out). Renderer: Home history list with localized status + friendly empty state (FR-7.3), startup resume offer (Folytatás/Új indítása, FR-7.2), `RestartBanner` on `agentError` (calm wording + one-click resume, raw detail tucked in an expander, FR-2.5 UI half); `TaskScreen` resume mode (skip form, `resumeRun` on mount). IPC + preload gained `listRuns/findResumable/resumeRun/dismissResume/restartAgent`; main wires the store into the runner, the new handlers, and `adapter.on("error") → agentError`. New E2E: kill mid-run (steerable, poll the persisted index for the resumable thread) → relaunch same userData → Folytatás → continues to SPIKE_OK success → history row shows "Kész". 126 vitest tests + 5 E2E + `pnpm typecheck`/`build`/`smoke` (real codex SPIKE_OK, now reporting 0.139.0 — mismatch logged non-fatally per FR-2.6) all green. Carry-forward: the two manual real-codex checks still need Peter at a signed-in session — quit app mid-run → relaunch → resume, and `kill -9` the codex pid → restart banner → one-click resume (Open Q4 steer reliability also still open from Phase 6). The codex-death→resume *UI navigation* (banner → restartAgent restores signed-in Home, the resumed run completes into history) is wired and unit-covered but its end-to-end feel is only provable in that manual run.
...
## Phase 8: The real app — Translate-Book end-to-end (dev mode) — ✅ COMPLETE (2026-06-24; real-codex baseline-book smoke run is carry-forward, like Phases 3–7's manual checks)

> Done (red-green TDD): **manifest schema** gained task `completion` (output globs, FR-1.3/6.3) + top-level `binaries` (FR-1.1), both validated naming the field; **`WorkspaceProvisioner` versioning** (FR-6.2) is now exercised — a bundle-version bump re-copies managed skill dirs while user files in the hidden workspace (a separate tree) survive; **`progress-watch.ts`** (`parseProgress` pure parser + `ProgressWatcher` that watches the workspace dir — robust to atomic-rename writes, deduped, fail-soft on malformed/missing files, PRD Open Q2); **`outputs.ts`** (`copyOutputs` — completion glob → *copied* not moved to `Documents/<AppName>/<job>/`, tiny `*`/`**`/`?` glob matcher, collision → `<job>-2`, FR-6.3); **`binary-health.ts`** (`checkBinaries` — PATH lookup with PATHEXT on win32, friendly Hungarian reinstall message naming the missing tools); **policy table** extended with the translate-book allowlist (python3/pandoc/ebook-convert accept, network/everything-else decline, FR-5.5). **`apps/translate-book/`**: `manifest.json` (file picker pdf/docx/epub + cover/export-name/address-select/instructions form, FR-5.5 policy, `binaries`, completion globs, purple branding), `sync-skill.sh` (vendors a pinned snapshot of the skill from `/home/megyo/projects/translate-book` — commit recorded in `skill/VENDOR.txt`; upstream stays source of truth) + the vendored `skill/` (ran the sync at commit c8b7c2e), and `skill-overlay/progress.md` appended to the vendored SKILL.md (the progress.json convention, kept out of upstream pending a coordinated PR). **Wiring**: `TaskEvent` gained `progress` + `finished.outputDir/outputFiles`; `TaskRunner` starts a `ProgressWatcher` per run and copies completion outputs to Documents on success (job name from export_name → file stem → task id); `RunView` renders a chunk-N-of-M `<progress>` bar and a success outputs list + "Mappa megnyitása" button; `openOutputDir` IPC (`shell.openPath`) wired through preload/main; binary-health gates boot (only for manifests declaring `binaries`, so echo-demo dev/E2E are untouched). 157 vitest tests (+31) + 5 E2E + `pnpm typecheck`/`build`/`smoke` (real codex, now 0.139.0 — mismatch logged non-fatally per FR-2.6, SPIKE_OK) all green; `t3code/` untouched, no Effect.ts dep.
>
> **Carry-forward (needs Peter at a signed-in browser with python3/pandoc/Calibre + a baseline book — the same class of manual checkpoint as Phases 3–7):** the actual dev-mode translate-book smoke run (`apps/translate-book` manifest + real codex → `standard-alice` → EPUB/DOCX/PDF/HTML in `Documents/Könyvfordító/<job>/`, "Open folder" works); mid-run app kill → resume completes without redoing finished chunks (leans on the skill's `run_state.json`); hostile-prompt denial stays friendly under the stricter policy; **PRD Open Q5** (sub-agent parallelism under app-server) resolved empirically and documented; and the coordinated backward-compatible upstream PR adding `progress.json` emission to the translate-book skill. A `pnpm smoke:translate-book` script was deliberately *not* added (it would be non-functional without the full toolchain + a baseline book; tracked here instead of shipping a misleading no-op).
...
## Phase 9.5: Visual polish — desktop-native look, manifest-driven theme — ✅ COMPLETE (2026-06-24)

> **Design direction — "warm editorial study" (revised 2026-06-24 after Peter found the first token pass too plain):** a literary, paper-and-ink desktop look fitting a book translator. Vendored real fonts (self-hosted woff2 under `shell-renderer/src/fonts/` + generated `fonts.css`, latin + latin-ext for Hungarian ő/ű, CSP relaxed to `font-src 'self'; img-src 'self' data:`): **Fraunces** (optical-sized display serif) for the wordmark/titles/big numbers + **Hanken Grotesk** (warm humanist sans) for UI. Warm paper neutrals, a brand-tinted multi-radial atmosphere + subtle SVG grain on `.app`, warm-tinted shadow ramp, and the manifest brand color as a confident accent (buttons, launcher icon tile + top-gradient bar, monogram, progress, chat bubbles, focus rings, status dots) so echo-demo (blue→green) and translate-book (violet→magenta) read distinctly while the craft stays constant. Added inline-SVG icons (`widgets/icons.tsx`, aria-hidden), staggered entrance/hover/modal motion (with `prefers-reduced-motion` guard), a branded login (monogram fallback + welcome line + tagline strings, hu/en), an app-like home (uppercase eyebrow, serif wordmark, pill settings, icon-tile launcher cards with a hover go-arrow, status pills, illustrated empty state), and run-status dots that reflect terminal state. Main process: window `backgroundColor` warm paper (no cold flash) + `autoHideMenuBar` (drops the coding-tool menu chrome, keeps edit accelerators); plus a Linux-only dev-noise fix (`GSETTINGS_BACKEND=memory` + `NO_AT_BRIDGE=1`) so `shell.openPath` on WSL stops flooding stderr with dconf/D-Bus warnings (no-op on the Windows/macOS targets). Verified by screenshots of both manifests (login/home/form/run). The earlier "boring tokens" pass below was the behavior-first baseline this builds on.
>
> Done (red-green TDD on the two testable seams, then the CSS by-eye with screenshot verification): **`themeTokens()`** gained a manifest-derived `--color-primary-contrast` (perceived-luminance/YIQ → white on dark brand colors, the app's ink on light ones, so a pale manifest keeps readable buttons; the Phase 9 `toEqual` token test was extended, +1 test for the light-primary flip). **`ParamForm`**'s submit is now the form's single `.primary` action (`button.primary`, asserted as the only primary in the form — styling hook + structural test). **`styles.css` was rewritten as a small, boring token-based design system**: static tokens on `:root` (spacing scale, radius, typography, a neutral surface/border/ink ramp, shadows, status colors) plus the brand color tokens seeded per-manifest onto `.app` from `themeTokens()`, so a second manifest restyles the whole app. Every previously-unstyled widget got real layout: param form (card, labelled field rows, checkbox-inline via `:has()`, file-picker as a secondary button + filename, red required hint, prominent primary "Indítás"), run view (status header, rounded brand progress bar, activity-feed cards, streamed-message panel, success/failed/cancelled terminal cards with the outputs list + "Mappa megnyitása"), chat pane (right-aligned brand message bubbles, composer row), the user-input modal and confirm dialog (backdrop + shadowed centered card + `.dialog-actions`), and login/home refinements (a branded login — product name/icon now passed from `App` via `boot.manifest.branding` — launcher cards with a brand accent bar and hover lift, settings menu, segmented locale switch). Base `button` is a neutral secondary; `.primary`/`.secondary`/`.danger`/`.link` compose; hover/focus-visible states use `color-mix`. **Markup changes were styling-hooks only** (no logic/IPC): `.primary`/`.secondary`/`.danger` classes on action buttons, `.file-field`/`.file-name`, `.task-actions`/`.dialog-actions` wrappers, the `Brand` block on Login. 178 vitest tests (+2) + 6 E2E + `pnpm typecheck`/`build`/`smoke` (real codex 0.139.0 → mismatch logged non-fatally per FR-2.6, SPIKE_OK) all green — the existing component tests are the guard that the reshuffle didn't change behavior. Verified by screenshotting both manifests through the mock peer (login/home/form): echo-demo (blue `#1f6feb`, "Echo Demo") and translate-book (purple `#7c3aed`, lavender bg, "Könyvfordító") render visibly distinct and desktop-native; the translate-book form reads as a finished product, not a raw page. `t3code/` untouched, no new deps, no Effect.ts. Out of scope as planned: no new behavior/IPC, no CSS framework, no custom per-task React components, no dark mode.
